<?php
	...
	class db_class extends db_connect{
		...
		public function delete_loan($loan_id) {
			$query = $this->conn->prepare("DELETE FROM `loan` WHERE `loan_id` = ?") or die($this->conn->error);
		...
		public function update_loan($loan_id, $ref_no, $ltype_id, $borrower_id, $purpose, $amount, $lplan_id, $status, $date_released) {
			try {
				$query = $this->conn->prepare("UPDATE `loan` SET `ltype_id`=?, `borrower_id`=?, `purpose`=?, `amount`=?, `lplan_id`=?, `status`=?, `date_released`=?, `ref_no`=? WHERE `loan_id`=?") or die($this->conn->error);
	
				// Bind parameters
				$query->bind_param("iisdiisii", $ltype_id, $borrower_id, $purpose, $amount, $lplan_id, $status, $date_released, $ref_no, $loan_id);
		...
		public function check_lplan($lplan) {
			$query = $this->conn->prepare("SELECT * FROM `loan_plan` WHERE `lplan_id` = ?") or die($this->conn->error);
			$query->bind_param("i", $lplan);
		...
		public function save_date_sched($loan_id, $due_date){
			$query = $this->conn->prepare("INSERT INTO `loan_schedule` (`loan_id`, `due_date`) VALUES (?, ?)") or die("Error preparing query: " . $this->conn->error);
			$query->bind_param("is", $loan_id, $due_date);
		
			if ($query->execute()) {
				$query->close();
				return true;
			} else {
				// Handle error more gracefully (log, display message, etc.)
